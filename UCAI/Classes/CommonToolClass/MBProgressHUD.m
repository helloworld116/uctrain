...
- (id)initWithFrame:(CGRect)frame {
	...
	if (self) {
		...
		self.square = NO;
        self.bgRGB = [UIColor colorWithRed:0.5 green:0.5 blue:0.5 alpha:self.opacity];
...
- (void)layoutSubviews {
    ...
    //indFrame.size.width = 37;
    //indFrame.size.height = 37;
...
- (void)drawRect:(CGRect)rect {
    ...
    //CGContextSetGrayFillColor(context, 0.5f, self.opacity);
    //CGContextSetRGBFillColor(context, 0.1, 0.5, 0.0, self.opacity);
    CGContextSetFillColorWithColor(context,self.bgRGB.CGColor);
